 ...
 */
export const flatten = (arr, childName = 'children') => {
    if (arr.length === 0) {
        return []
    }
    ...
    for (let i = 0; i < oldArr.length; i++) {
        if (oldArr[i][childName]) { // childName存在
 ...
 */
export const treeData = (arr, childName = 'children') => {
    if (arr.length === 0) {
        return []
    }
...
/**
 * @param {Array} arr  左侧菜单源数据
 * @param {String} childName 自定义子集的属性名称，默认children
 */
export const getAllButtons = (arr, childName = 'children') => {
    if (arr.length === 0) {
        return []
    }
    let oldArr = JSON.parse(JSON.stringify(arr)) // 对源数据深度克隆
    let newArr = []
    for (let i = 0; i < oldArr.length; i++) {
        if (oldArr[i].buttons) { // childName存在
            newArr.push(...oldArr[i].buttons)
        }
        if (oldArr[i][childName]) { // childName存在
            newArr.push(...getAllButtons(oldArr[i][childName]))
        }
    }
    return newArr
}
