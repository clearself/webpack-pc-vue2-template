<template>
    <div class="topology" v-loading="loading">
        <el-row :class="graphBg" style="height: 100%" @contextmenu.native.prevent>
            <el-col class="graph-container1" :span="24" ref="graphContainer">
                <div id="mount-topology"></div>
...
import theme from './theme/index-bigScreen'
import initGraph from './graph/bigScreen'
...
import {
    mapBigGraph
} from '@/gplot/cc-topology/src/utils/asset-to-bigscreen'
...
export default {
        ...
        return {
            ...
            edgeShapeList: [
                ...
                },
                {
                    guid: 'cc-cubic',
                    label: '曲线',
                    class: 'iconfont icon-flow-curve'
                },
                {
                    guid: 'cc-dot',
                    label: '动线',
    ...
    methods: {
        ...
        handleSelect(item) {
            ...
            this.selectedNodeParams.appConfig.ip = item.ip || ''
            this.selectedNodeParams.appConfig.port = item.port || ''
        },
        toggleCollapse() {
        ...
        initTopo(graphData) {
            ...
            let graphHeight = graphContainer.clientHeight - 10
            // Plugins
            let plugins = []
            let modes = {
                default: [
            ...
            self.graph.refresh()
            self.graph.on('node:mouseenter', env => {
                console.log(env, '我是节点111111111111111')
                console.log(env.item._cfg.model.appConfig.ip, '')
                let graph = document.getElementsByClassName('graph-container1')
                console.log(graph)
                let a = document.createElement('div')
                a.innerHTML = `ip: ${env.item._cfg.model.appConfig.ip}`
                a.classList.add('nodeInfo')
                a.setAttribute('style', `top: ${env.canvasY}px; left: ${env.canvasX}px`)
                graph[0].appendChild(a)
            })
            self.graph.on('node:mouseout', env => {
                let graph = document.getElementsByClassName('graph-container1')
                let a = document.getElementsByClassName('nodeInfo')
                graph[0].removeChild(a[0])
            })
            // self.graph.zoomTo(1)
            self.autoZoomHandler()
            this.enableMinimapHandler()
        },
        /* 自动布局 */
        ...
        changeEdgeShapeHandler(edgeShape) {
            this.currentEdgeShape = _.find(this.edgeShapeList, {
                guid: edgeShape
            })
        ...
        redoHandler() {
            if (this.undoCount > 0) {
                let key = `graph_history_${
                    this.historyIndex - this.undoCount + 1
                }`
                let historyData = this.getHistoryData(key)
                this.changeGraphData(JSON.parse(historyData))
                this.undoCount -= 1
        ...
        removeComboHandler() {
            ...
            if (this.selectedCombo) {
                ...
                let { nodes: subNodes, combos: subCombos } =
                ...
                switch (themeValue) {
                    ...
                    case 'whiteStyle':
                        ...
                        for (let i = 0, len = edges.length; i < len; i++) {
                            ...
                            let edgeModel = edge.getModel()
                            edgeModel.edgeModel =
                                graph.$T.edgeStyle.edgeLabelCfg
                            edgeModel.style = graph.$T.edgeStyle.default
        ...
        selectedComboParams: {
            ...
            handler: function(newVal, oldVal) {
                ...
                if (
                    ...
                    selectedComboModel.labelCfg.position ===
