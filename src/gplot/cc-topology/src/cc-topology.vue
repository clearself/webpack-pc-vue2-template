<template>
    <div class="topology" v-loading="loading">
        ...
        <el-row :class="graphBg" style="height: 100%" @contextmenu.native.prevent>
            <!-- toolbar -->
            <div v-if="toolbarShow && ['add', 'edit'].includes(mode)">
            ...
            <div class="graph-pannel" v-if="graphMode === 'edit' && graphPanelShow === true" :span="4" :offset="1">
                <div v-if="currentFocus === 'node'">
                    ...
                    <div class="pannel-body">
                        <div>
                            <span><sup style="color:red;">*</sup>资产名称</span>
                            <el-autocomplete
                                class="params-input"
                                style="width:100%"
                        ...
                        <div>
                            <span><sup style="color:red;">*</sup>节点IP</span>
                            <el-input class="params-input" oninput ="value=value.replace(/[^0-9.:]/g,'')"  @blur="selectedNodeParams.appConfig.ip = $event.target.value" v-model="selectedNodeParams.appConfig.ip" size="mini"></el-input>
                        </div>
                        <div>
                            <span>节点端口</span>
                            <el-input class="params-input" oninput ="value=value.replace(/[^0-9]/g,'')"  @blur="selectedNodeParams.appConfig.port = $event.target.value" v-model="selectedNodeParams.appConfig.port" size="mini"></el-input>
                        </div>
                    </div>
                </div>
        ...
        addNode(clientX, clientY, nodeType) {
            ...
            if (graph && !graph.destroyed) {
                ...
                let node = graph.addItem('node', {
                    ...
                    orNew: true, // 只要是拖进来的新建的节点 orNew就是true
