<template>
    <div class="wrapper">
        <div class="ub ub-ac">
            <p class="filter-title" style="color:rgba(0, 0, 0, 0.9);font-size:12px; margin-top: -10px;margin-left: 20px;margin-right: 9px">过滤条件:</p>
            <div class="condition-item ub ub-pj ub-ac" v-for="(item, index) in conditionData" :key="index">
                ...
                <i class="el-icon-close"  @click.stop="deleteCondition(index)" style="line-height:23px"></i>
            </div>
            <div class="add-btn ub ub-ac"  @click="addCondition">
                <i class="el-icon-plus" style="line-height:23px"></i>
                <p>添加过滤条件</p>
            </div>
        </div>
    </div>
