<template>
    <div>
        <el-dialog title="历史查询记录" :visible.sync="recordDialog" width="900px" custom-class="common-dialog" :before-close="cancel" class="see-dialog">
                        ...
                        <el-table ref="multipleTable" v-loading="loading" class='bigTable' :data="tableData" border stripe tooltip-effect="dark" :height="310">
                            ...
                            <el-table-column label="查询条件">
                                <template slot-scope="scope">
                                    ...
                                    <div class="ub">
                                        <!-- {{ scope.row.inputStr }} -->
                                        <div style="width: 90px;color:#0052d9">检索时间：</div> <div style="width: 250px;word-break:break-all;word-wrap: break-word;overflow:auto"> {{unitTime(scope.row.time,scope.row.timeStatus)}}{{scope.row.timeStatus?unitText(scope.row.timeStatus):''}}</div>
                                    </div>
                                    <div class="ub">
                                        <div style="width: 90px;color:#0052d9">搜索条件：</div> <div style="width: 250px;word-break:break-all;word-wrap: break-word;overflow:auto"> {{ scope.row.sqlStr }}</div>
                                    </div>
                                    <div class="ub">
                                        <div style="width: 90px;color:#0052d9">过滤条件：</div> <div style="width: 250px;word-break:break-all;word-wrap: break-word;overflow:auto"> {{ scope.row.groupStr }}</div>
                                    </div>
                                </template>
                            </el-table-column>
...
// import { pageSearchHistory, deleteSearchHistory } from '@/server/stream_data_center/data_manage.js'
import { pageSearchHistory, deleteSearchHistory } from '@/server/alarm_log/api.js'
export default {
    props: ['recordDialog'],
    components: {
        Classify
