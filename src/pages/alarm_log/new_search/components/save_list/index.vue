                        ...
                        <div>
                            <el-table
                                        ...
                                        <div>
                                            ...
                                            >
                                                ...
                                                <p
                                                    v-else
                                                    class="highlighted"
                                                    :style="['ip', 'url', '域名'].some(keyWord => item.field.toLowerCase().includes(keyWord)) ? 'cursor: pointer; text-decoration: underline; color: #03a5dd;' : 'cursor: default; text-decoration: unset; color:  rgba(0, 0, 0, 0.9);'"
                                                    slot="reference"
                                ...
                                <el-table-column label="操作" align="center" width="200" fixed="right" class-name="deepBg">
                                    <template slot-scope="scope">
                                        <el-button :disabled="scope.row.nftData!==0" @click="createPackTask(scope.row)" type="text" size="small">创建PCAP包</el-button>
...
import {
    ...
    updateStashFields,
    clearStashFields
} from '@/server/alarm_log/stash.js'
export default {
    name: 'SaveList',
    components: {
        CustomDate,
                        ...
                        for (const resKey in res.info) {
                            if (res[resKey] !== '') {
                                ...
                                if (item) {
                                    arr.push({
                                        name: item.name,
                                        value: res.info[resKey]
                                    })
                                    obj[item.field] = res.info[resKey]
                                }
                            }
                        }
                        if (messageObj) {
