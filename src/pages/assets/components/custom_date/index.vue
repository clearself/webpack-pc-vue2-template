...
<script>
import { saveLogTime, getLogTime, deleteLogTime } from '@/server/data_manage/new_search.js'
export default {
    name: 'CustomDate',
    props: {
        appendToBody: {
    ...
    methods: {
        ...
        handleOther(data, index) {
            ...
            let num = -data.time
            this.$setsessionStorage('temporaryAssets', { actualTime: data.time, timeUnit: data.timeUnit + 1 })
            let endTime = this.$moment().format('YYYY-MM-DD HH:mm:ss')
        ...
        handleRelative(data, index) {
            this.$setsessionStorage('temporaryAssets', { actualTime: '', timeUnit: '' })
        ...
        addTime() {
            ...
            saveLogTime(data).then(res => {
                ...
                this.$setsessionStorage('temporaryAssets', { actualTime: this.actualTime, timeUnit: this.timeUnit + 1 })
                this.handleUse()
                this.$refs.datePopver.doClose()
                setTimeout(() => {
                    this.getLogTimeData()
