<template>
    <div style="width:100%;height:100%" ref="barRef"><div style="width:100%;height: 100%;" ref="myEcharts" class="echarts"></div></div>
</template>
...
export default {
    name: 'Bar',
    props: ['allFieldList', 'barData', 'chatBarOption', 'seq', 'type'], // 3日志， 5事件， 6资产， 7漏洞
    data() {
        return {
            myChart: null,
            loading: true,
    ...
    methods: {
        ...
        renderChart() {
            ...
            // 第四种，有xy，有细分，有度量。
            // 细分字段
            let sub = this.barOption.attr.subdivision[0]
            // x轴字段
            let x = this.barOption.xData.field[0]
            // y轴字段
            let y = this.barOption.yData.field[0]
            // 颜色区分字段
            let color = this.barOption.attr.color[0]
            // 度量字段
            let measureList = this.barOption.measureList
            // 是否有度量
            ...
            let series = []
            // 是否堆叠
            let isStack = this.barOption.attr.type == 2
            // 生成颜色数据，只有颜色字段值与细分字段值相同，或者颜色字段值是度量名称，且存在度量列表需要处理颜色数据，否则，颜色为默认不需要处理
            let colors = {}
            // 特殊处理，为某些特定字段配置颜色
            let colorList = this.barOption.attr.colorSet
            console.log(color, sub, hasMeasure, x, this.chartData)
            if (color && color == sub) {
                this.chartData.forEach(item => {
                    if (!(item[sub] in colors)) {
                        ...
                        let name = item[sub]
                        if (color == 'level') {
                            colors[name] = colorList.length && colorList.filter(cl => cl.name == name)[0].color
                        } else {
            ...
            console.log(xData)
            console.log(series)
            console.log(legendSort)
            console.log(color)
            let xname = this.getNameByCode(x)
            let colorname = color ? this.getNameByCode(color) : ''
            console.log(xname)
            if (xname == '事件等级' && colorname == '事件等级' && !sub && !hasMeasure && colorList.length && series.length) {
                let tmpSe = []
                // 事件等级，无细分无度量的情况下特殊处理每个柱子颜色
                series[0].data.map((item, index) => {
                    let tx = xData[index]
                    let color = colorList.filter(cl => cl.name == tx)[0].color
                    let obj = {
                        value: item,
                        itemStyle: {
                            color: color
            ...
            let clientWidth = this.$refs.barRef.clientWidth
            let fontSize = 12
            ...
            let option = {
                tooltip: {
                    ...
                    extraCssText: 'white-space:pre-line;word-break: break-all;',
                    confine: true
                },
                grid: {
                    show: false,
                        ...
                        children: [
                            {
                                ...
                                style: {
                                    fill: '#333',
                                    text: this.seq ? this.getNameByCode(y) + '（' + this.aggTypeList[this.barOption.yData.aggType] + '）' : this.getNameByCode(x),
                        ...
                        children: [
                            {
                                ...
                                style: {
                                    fill: '#333',
                                    text: this.seq ? this.getNameByCode(x) : this.getNameByCode(y) + '（' + this.aggTypeList[this.barOption.yData.aggType] + '）',
                                    font: '14px Microsoft YaHei'
                                }
                            }
                        ]
