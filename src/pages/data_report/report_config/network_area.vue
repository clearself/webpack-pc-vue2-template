<template>
    <div class="event event-wrapper network_area" style="min-height:auto!important;">
        <div>
            <div class="ub ub-pj w100 mb-1">
                <div class="list-tips">外部网络区域</div>
                <div>
            ...
            <el-table
                ...
                <el-table-column prop="name" label="外部网络区域" show-overflow-tooltip>
                </el-table-column>
        ...
        <el-dialog custom-class="common-dialog" :title="type === 'add' ? '添加数据' : '编辑数据'" width="700px" :visible.sync="addEditDialog">
            <el-form ref="formData" :model="formData" label-position="top" :rules="rules">
                <el-row :gutter="50">
                    <el-col :span="12">
                        <el-form-item size="small" label="外部网络区域：" prop="code" :label-width="formLabelWidth">
                            <el-select size="small" v-model="formData.code" placeholder="请选择" style="width: 100%">
                                <el-option v-for="item in area_data" :key="item.value" :label="item.label" :value="item.value"></el-option>
                            </el-select>
                        </el-form-item>
                    </el-col>
                    <el-col :span="12">
                    ...
                    <el-col :span="24">
                        <el-form-item size="small" label="IP范围：" prop="ipRange" :label-width="formLabelWidth">
                            <el-input :rows="4" size="small" v-model="formData.ipRange" placeholder="请输入内容" autocomplete="off" type="textarea"></el-input>
                        </el-form-item>
                    </el-col>
                </el-row>
            </el-form>
...
export default {
    ...
    methods: {
        ...
        addEquipment() {
            let cur_area_data = area_data.filter(item => item.value === this.formData.code)
            let data = {
                queryData: {},
                paramsData: {
                    type: 4,
                    name: cur_area_data.length > 0 ? cur_area_data[0].label : '',
