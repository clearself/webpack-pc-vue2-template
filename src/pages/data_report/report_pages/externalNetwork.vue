    ...
    <div class="event reporting_platform" style="box-sizing: border-box;overflow-y: hidden;">
        <div ref="searchWrap" class="ub w100" style="box-sizing: border-box;">
            <SearchTop class="seaTopBox" @search="searchData" @reset="reset"  @isOpen="isOpen1">
                ...
                <template v-if="tag == 22 || tag == 23 || tag == 24">
                    <el-col :md="12" :lg="8" :xl="6">
                        <el-form :model="get_params">
        ...
        <div class="list-container" style="overflow-y: auto;height: calc(100% - 70px)">
            <div class="ub ub-pj w100 mb-1">
                <div class="list-tips">待上报数据</div>
                <div>
                    <el-checkbox v-model="selAll">全选</el-checkbox>
                    ...
                    <!--                    <opt-btn style="margin-right: 10px;" title="点击查看帮助手册" icon="el-icon-question" @click="$pushRouter('/help_manual/help_reported_data')"></opt-btn>-->
                    <add-btn v-if="tag != 21" isReuse style="margin-left: 10px;" icon="el-icon-plus" title="添加数据" @click="toAdd" />
                    <add-btn isReuse style="margin-left: 10px;" icon="el-icon-upload" title="上 报" @click="handleReportAll" />
                    <add-btn isReuse style="margin-left: 10px;" icon="el-icon-check" title="审 核" @click="handleCheckAll" />
                    <!--                    <add-btn isReuse style="margin-left: 10px;" icon="el-icon-download" @click="handleImp" title="导 入" />-->

                    <add-btn v-if="importOpt.templateUrl" isReuse style="margin-left: 10px;" icon="iconfont icon-daochu" title="下载模板" @click="dwTemplate" />

                    <el-upload
                        v-if="importOpt.url"
                        :headers="header"
                        :action="importOpt.url"
                        name="file"
                        accept=".xlsx,.xlsx"
                        :file-list="fileList"
                        ...
                        <el-button  size="small" style="margin-left:10px" icon="iconfont icon-daoru" type="primary">导 入</el-button>
                    </el-upload>

                    <add-btn isReuse style="margin-left: 10px;" icon="iconfont icon-daochu" @click="handleExp" title="导 出" />
            ...
            <el-table
                ...
                <template v-for="(it) in tableHeaders">
                    <el-table-column v-if="it.prop == 'date' && tag== 21" :key="it.prop" :label="it.label" align="center">
                ...
                <el-table-column align="center" label="操作" width="180" fixed="right" class-name="deepBg">
                    <template slot-scope="scope">
                        <div>
                            <div>
                                <el-button :disabled="scope.row.approve_status!=1" size="small" type="text" @click="handleReport(scope.row)">上报</el-button>
                                <el-button :disabled="scope.row.approve_status!=0" size="small" type="text" @click="handleCheck(scope.row)">审核</el-button>
                                <el-button v-if="tag != 21" :disabled="scope.row.source_type!=2 || scope.row.approve_status==2" size="small" type="text" @click="toEdit(scope.row)">编辑</el-button>
                                <el-button class="delBtn" :disabled="scope.row.source_type!=2" size="small" type="text" @click="handleDel(scope.row)">删除</el-button>
                            </div>
                        </div>
                    </template>
                </el-table-column>
        ...
        <externalNetworkDia v-if="tag == 22" data_type="S01" :beAttackOpt="beAttackOpt" :attackOpt="attackOpt" ref="editDia" @refresh="getList"></externalNetworkDia>
        <internalNetworkDia v-else-if="tag == 23" data_type="S02" :beAttackOpt="beAttackOpt" :attackOpt="attackOpt" ref="editDia" @refresh="getList"></internalNetworkDia>
        <certificateNetworkDia v-else-if="tag == 24" data_type="S03" :beAttackOpt="beAttackOpt" :attackOpt="attackOpt" ref="editDia" @refresh="getList"></certificateNetworkDia>
        <abnormalOutreachDia v-else-if="tag == 25" data_type="S04" :beAttackOpt="beAttackOpt" :attackOpt="attackOpt" ref="editDia" @refresh="getList"></abnormalOutreachDia>
        <ddocDia v-else-if="tag == 26" data_type="S05" :beAttackOpt="beAttackOpt" :attackOpt="attackOpt" ref="editDia" @refresh="getList"></ddocDia>
        <maliciousCodeDia v-else-if="tag == 27" data_type="S06" :beAttackOpt="beAttackOpt" :attackOpt="attackOpt" ref="editDia" @refresh="getList"></maliciousCodeDia>
        <maliciousMailDia v-else-if="tag == 17" data_type="S07" :beAttackOpt="beAttackOpt" :attackOpt="attackOpt" ref="editDia" @refresh="getList"></maliciousMailDia>
        <phishingWebsiteDia v-else-if="tag == 18" data_type="S08" :beAttackOpt="beAttackOpt" :attackOpt="attackOpt" ref="editDia" @refresh="getList"></phishingWebsiteDia>
        <fakeAppDia v-else-if="tag == 19" data_type="S09" :beAttackOpt="beAttackOpt" :attackOpt="attackOpt" ref="editDia" @refresh="getList"></fakeAppDia>
        <websiteTamperingDia v-else-if="tag == 20" data_type="S10" :beAttackOpt="beAttackOpt" :attackOpt="attackOpt" ref="editDia" @refresh="getList"></websiteTamperingDia>
    </div>
...
import externalNetworkDia from './dia/externalNetworkDia.vue'
import internalNetworkDia from './dia/internalNetworkDia.vue'
import certificateNetworkDia from './dia/certificateNetworkDia.vue'
import abnormalOutreachDia from './dia/abnormalOutreachDia.vue'
import ddocDia from './dia/ddocDia.vue'
import maliciousCodeDia from './dia/maliciousCodeDia.vue'
import maliciousMailDia from './dia/maliciousMailDia.vue'
import phishingWebsiteDia from './dia/phishingWebsiteDia.vue'
import fakeAppDia from './dia/fakeAppDia.vue'
import websiteTamperingDia from './dia/websiteTamperingDia.vue'
...
export default {
    ...
    methods: {
        ...
        async dwTemplate() {
            try {
                await totalMethods.downloadTemReq(this.importOpt.templateUrl)
                this.$message({
                    message: '下载模板成功',
                    type: 'success'
                })
        ...
        handleCheckAll() {
            ...
            if (this.multipleSelection.length > 0) {
                ...
                if (this.multipleSelection.some(item => (item.approve_status != 0))) {
                    this.$message({
                        message: '请选择未审核的数据！',
                        type: 'warning'
                    })
        ...
        checkConfimAll() {
            if (!this.selAll) {
                ...
                let paramsData = {
                    ...
                    startTime: this.search_time === null ? '' : this.search_time[0],
                    endTime: this.search_time === null ? '' : this.search_time[1],
                    approveStatusCheckAll: this.examineFormEditAll.status
                }
        ...
        handleReportAll() {
            ...
            if (this.multipleSelection.length > 0) {
                if (this.multipleSelection.some(item => (item.approve_status != 1))) {
                    this.$message({
                        message: '请选择审核成功的数据！',
                        type: 'warning'
                    })
        ...
        handleDelAll() {
            ...
            if (this.multipleSelection.length > 0) {
                if (this.multipleSelection.some(item => (item.source_type != 2))) {
                    this.$message({
                        message: '请选择允许删除的数据！',
                        type: 'warning'
                    })
        ...
        totalDel() {
            this.$confirm('确认要删除查询到的所有可以删除的数据吗?', '警告', {
        ...
        totalReport() {
            this.$confirm('确认要上报查询到的所有可以上报的数据吗?', '警告', {
                confirmButtonText: '确定',
                cancelButtonText: '取消',
                type: 'warning'
            }).then(async() => {
        ...
        totalAudit() {
            this.$confirm('确认要审核查询到的所有可以审核的数据吗?', '警告', {
                confirmButtonText: '确定',
                cancelButtonText: '取消',
                type: 'warning'
            }).then(async() => {
    ...
    created() {
        if (this.$route.name == 'externalNetwork') {
            this.tag = 22
            pageMethod = totalMethods.externalNetworkPageReq
            ...
            this.getDict()
        } else if (this.$route.name == 'internalNetwork') {
            this.tag = 23
            pageMethod = totalMethods.internalNetworkPageReq
            ...
            this.getDict()
        } else if (this.$route.name == 'certificateNetwork') {
            this.tag = 24
            pageMethod = totalMethods.certificateNetworkPageReq
        ...
        } else if (this.$route.name == 'abnormalOutreach') {
            this.tag = 25
            pageMethod = totalMethods.abnormalOutreachPageReq
            ...
            this.getDict()
        } else if (this.$route.name == 'ddoc') {
            this.tag = 26
            pageMethod = totalMethods.ddocPageReq
            ...
            this.importOpt = {
                url: '/api/data-report/manage/ddosattack/importData',
                templateUrl: '/data-report/manage/ddosattack/download'
            }
            this.getDict()
        } else if (this.$route.name == 'maliciousCode') {
            this.tag = 27
            pageMethod = totalMethods.maliciousCodePageReq
            ...
            this.get_Area()
            this.getDict()
        } else if (this.$route.name == 'maliciousMail') {
            this.tag = 17
            pageMethod = totalMethods.maliciousMailPageReq
        ...
        } else if (this.$route.name == 'phishingWebsite') {
            this.tag = 18
            pageMethod = totalMethods.fakePhiWebPageReq
            ...
            this.importOpt = {
                url: '/api/data-report/manage/fakePhishingWebsite/importData',
                templateUrl: '/data-report/manage/fakePhishingWebsite/download'
            }
        } else if (this.$route.name == 'fakeApp') {
            this.tag = 19
            pageMethod = totalMethods.fakeAppPageReq
            ...
            this.importOpt = {
                url: '/api/data-report/manage/fakeApp/importData',
                templateUrl: '/data-report/manage/fakeApp/download'
            }
        } else if (this.$route.name == 'websiteTampering') {
            this.tag = 20
            pageMethod = totalMethods.webTamPageReq
            ...
            this.importOpt = {
                url: '/api/data-report/manage/websiteTampering/importData',
                templateUrl: '/data-report/manage/websiteTampering/download'
            }
        } else if (this.$route.name == 'reportStatistics') {
            this.tag = 21
            pageMethod = totalMethods.staVerPageReq
