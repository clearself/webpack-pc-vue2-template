    ...
    <div class="list">
            ...
            <div class="event-content w100" :style="{'height':tabHeight+'px'}" style="overflow-y:auto">
                <div class="ub ub-pj w100 ub-ac" style="padding:10px">
                    ...
                    <div>
                        ...
                        <el-button @click="validMatch('form')" size="small" type="primary">修改</el-button>
                    </div>
                </div>
                <el-form ref="form" :model="form" :rules="formRules" label-width="100px"  label-position="top">
                    <div class="base-box">
                        <div>
                            <div>
                                <div class="ub ub-pj">
                                    ...
                                    <el-form-item label="演练赛制：" size="mini" style="width: calc((100% - 168px) / 3);">
                                ...
                                <div class="ub ub-pj">
                                    <el-form-item label="演练时间：" prop="matchTime" size="small" style="width: calc((100% - 168px) / 3);">
                                        <el-date-picker style="width: 100%" v-model.trim="form.matchTime" value-format="yyyy-MM-dd HH:mm:ss" type="datetimerange" range-separator="至" start-placeholder="开始日期" end-placeholder="结束日期">
                                        </el-date-picker>
                                    </el-form-item>
                                    <el-form-item label="演练简介：" style="width: calc((100% - 168px) / 3);">
                        ...
                        <el-form-item style="display: block;" label="演练规则：" label-position="left">
                            <el-upload :limit="1" ref="upload" action="/api/base-server/file/upload" :before-upload="beforeUpload" name="fileUpload" :data="uploadToken" :on-success="uploadFileSuccess" :on-error="uploadFileError" :on-exceed="limitFun">
                                <span style="color:rgba(0,0,0,.7);">{{form.ruleFileName?form.ruleFileName.split('/')[3]:''}}</span>
                    ...
                    <div class="base-box">
                        <div class="ub ub-ac ub-pj w100 match-title">
                            <div style="color:rgba(0,0,0,.7);font-size:12px">参赛裁判：</div>
                            <div>
                                <el-button class="add" type="text" icon="el-icon-plus" @click="refereeListShow" size="mini">添加裁判</el-button>
                            </div>
                        </div>
                        <div>
                            <el-table :data="refereeData" border class='bigTable'>
                        ...
                        <div>
                            <el-table :data="teamList" border class='bigTable'>
                                ...
                                <el-table-column prop="player_num" label="战队人数">
                                </el-table-column>
        ...
        <el-dialog :visible.sync="refereeDialog" width="790px" custom-class="common-dialog" title="裁判列表">
            <div class="ub ub-ac ub-f1 ub-pj" style="margin: 10px 0;">
                ...
                <div>
                    <el-button icon="el-icon-refresh" type="text" @click="refreshReferee(1)" size="mini">刷新</el-button>
                </div>
            </div>
            <el-table :data="referee_data" v-loading="referee_loading" ref="multipleTable" :row-key="(row)=>{ return row.id}" border stripe @select="handleRefereeSelectionChange" @select-all="handleRefereeSelectionChangeAll" class='bigTable' :row-class-name="tableRowClassName">
        ...
        <el-dialog title="提示" :visible.sync="revPlarersDialog" width="30%" custom-class="attendance-dialog">
            <span>{{revPlarersStr}}</span>
            <span slot="footer" class="dialog-footer">
                <searchBtn title="确 定" @click="sure_team" />
                <cancel-btn title="取 消" style="margin-left: 10px;" @click="revPlarersDialog = false" />
            </span>
        </el-dialog>
        <DeleteDialog
        ...
        <!-- 编辑战队 -->
        <el-dialog title="编辑人员" :visible.sync="editTeamDialog" width="700px" custom-class="attendance-dialog">
            <el-form ref="editTeamForm" :model="editTeamForm" :rules="createTeamRules" label-width="100px"  custom-class="common-dialog">
...
export default {
    ...
    methods: {
        ...
        getUsers(val) {
            this.storeIdSelect = val
            this.transfer_data = []
            let data = {
                queryData: {},
                paramsData: {
                    id: val,
                    page: this.createTeamForm.page,
                    size: this.createTeamForm.size,
                    userName: ''
                }
            }
        ...
        get_referee() {
            ...
            get_match_referee(data).then((res) => {
                ...
                this.referee_totalPages = res.pages
        ...
        refereeListShow() {
            console.log(this.$store.state.referee.data)
        ...
        selectSeeForm(val) {
            this.seeForm = this.seeForm.map(item => {
                item.isActive = false
                return item
            })
            val.isActive = true
            this.form.seeForm = val.type
        },
        selectSeeTheme(val) {
            this.seeTheme = this.seeTheme.map(item => {
                item.isActive = false
                return item
            })
            val.isActive = true
            this.form.seeTheme = val.type
        },
        selectMatchForm(val) {
            this.matchForm = this.matchForm.map(item => {
                item.isActive = false
                return item
            })
            val.isActive = true
            this.form.matchForm = val.type
        },
        selectMatchRule(val) {
            this.matchRule = this.matchRule.map(item => {
                item.isActive = false
                return item
            })
            val.isActive = true
            this.form.matchRule = val.type
        },
