    ...
    <div class="list">
        <div class="ub mb-1" style="justify-content: space-around;">
            <div class="ub ub-f1 chart-box mr-1">
                <div class="ub ub-f1 ub-ac statistic-item">
                    <div class="ub ub-ver">
                        <div class="ml-2" style="font-size:12px"><i class="iconfont icon-dengjizichan" style="font-size:12px;color:#efcef7;margin-right:5px"></i>攻击总量</div>
            ...
            <div class="ub ub-f1 chart-box mr-1">
                <div class="ub ub-f1 ub-ac statistic-item">
                    <div class="ub ub-ver">
                        <div class="ml-2" style="font-size:12px"><i class="iconfont icon-gongjifangcheng-shenhe" style="font-size:12px;color:#61cfa9;margin-right:5px"></i>已审核攻击</div>
            ...
            <div class="ub ub-f1 chart-box mr-1">
                <div class="ub ub-f1 ub-ac statistic-item">
                    <div class="ub ub-ver">
                        <div class="ml-2" style="font-size:12px"><i class="iconfont icon-gongjifangcheng-daishenhe" style="font-size:12px;color:#f29960;margin-right:5px"></i>待审核攻击</div>
            ...
            <div class="ub ub-f1 chart-box mr-1">
                <div class="ub ub-f1 ub-ac statistic-item">
                    <div class="ub ub-ver">
                        <div class="ml-2" style="font-size:12px"><i class="iconfont icon-gongjifangchengguo-defen" style="font-size:12px;color:#4a89f0;margin-right:5px"></i>攻击得分</div>
            ...
            <div class="ub ub-f1 chart-box">
                <div class="ub ub-f1 ub-ac statistic-item">
                    <div class="ub ub-ver">
                        <div class="ml-2" style="font-size:12px"><i class="el-icon-sort" style="font-size:12px;color:#e3515c;margin-right:5px"></i>攻击得分</div>
                        <div style="color:#e3515c;margin-left:35px;font-size: 20px;margin-top:5px">{{get_params.match!=''? top_data.num5: '-'}}</div>
                    </div>
                    <div class="ub-f1 text-right" style="padding-right: 0px;padding-top: 3px;">
                        <img style="height:82px" src="../../assets/img/defense_drill/defense_top5.png" alt="">
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="list-container" style="margin-top:-2px;box-shadow:none">
            <div class="ub ub-pj w100 mb-1 ub-ac">
                <div class="list-tips">攻击方成果</div>
                <div>
                    <el-button icon="el-icon-plus" size="small" type="primary" @click="showCreateRecord" v-per="['defense_gj_add']">添加成果</el-button>
                    <el-button size="small" type="primary" @click="downData" icon="iconfont icon-daochu" v-per="['defense_gj_exp']">导出成果</el-button>
                    <el-button style="margin-left: 10px;" icon="iconfont icon-daoru" title="成果导入" @click="uploadDialog=true" size="small" type="primary" v-per="['defense_gj_imp']">导入成果</el-button>
                    <el-button icon="el-icon-download" @click="downLoad" size="small" type="primary"  style="margin-left: 10px;" v-per="['defense_gj_download']">下载模板</el-button>
                    <el-button size="small" type="danger" style="margin-left:10px" icon="el-icon-delete" @click="deleteAll" v-per="['defense_gj_del']">删除</el-button>
                </div>
            </div>
            <el-table
                class="bigTable"
                ...
                <el-table-column prop="last_state" label="状态" width="100" :render-header="renderHeader">
                    <template slot-scope="scope">
                        <div>
                            <div v-if="scope.row.last_state_id==1" class="ub ub-ac match-state one">
                                <div style="width:80px;text-align:center">{{scope.row.last_state}}</div>
                            </div>
                            <div v-if="scope.row.last_state_id==2" class="ub ub-ac match-state two">
                                <div style="width:80px;text-align:center">{{scope.row.last_state}}</div>
                            </div>
                            <div v-if="scope.row.last_state_id==3" class="ub ub-ac match-state three">
                                <div style="width:80px;text-align:center">{{scope.row.last_state}}</div>
                            </div>
                            <div v-if="scope.row.last_state_id==4" class="ub ub-ac match-state four">
                                <div style="width:80px;text-align:center">{{scope.row.last_state}}</div>
                            </div>
                        </div>
                    </template>
                ...
                <el-table-column show-overflow-tooltip prop="attack_time" label="攻击时间" width="170px">
                </el-table-column>
                ...
                <el-table-column align="center" fixed="right" label="操作" width="160px" class-name="deepBg">
                    <template slot-scope="scope">
                        <el-button @click="handleAttackSee(scope.row)" size="small" type="text" v-per="['defense_gj_view']">查看</el-button>
                        <el-button :disabled="scope.row.isCheck" @click="handleAttackEdit(scope.row)" size="small" type="text" v-per="['defense_gj_edit']">编辑</el-button>
                        <el-button :disabled="scope.row.isCheck" @click="handleAttackdelete(scope.row)" size="small" type="text" v-per="['defense_gj_del']">删除</el-button>
                    </template>
                </el-table-column>
            </el-table>
        ...
        <el-dialog title="成果导入" :visible.sync="uploadDialog" width="700px" custom-class="common-dialog">
            <el-form ref="uploadForm" :model="uploadForm" :rules="uploadFormRule" label-width="100px" label-position="top">
                ...
                <el-form-item style="margin-top: 30px;" label="上传附件:">
                    <el-upload :headers="header" ref="upload" action="api/defense-drill/manage/record/importExcel" :file-list="fileList" :data="{'type':1,'id':uploadForm.match}" :auto-upload="false" multiple name="file" :on-success="uploadSuccess">
...
export default {
    ...
    methods: {
        ...
        downData() {
            recordExportExcel({ queryData: { type: 1 }, paramsData: {}}).then(res => {
                console.log(res)
            }).catch(err => {
                console.log(err)
            })
        ...
        downLoad() {
            recordTemplate({ queryData: { type: 1 }, paramsData: {}}).then(res => {
                console.log(res)
            }).catch(error => {
                console.log(error)
            })
            ...
            get_records(data).then((res) => {
                ...
                if (list.length > 0) {
                    list.forEach((item, index) => {
                        ...
                        obj.scoring_reason = this.$NullStr(item.scoring_reason)
                        ...
                        if (item.refereeInfo && item.refereeInfo.length > 0) {
                            ...
                            let selectedRefense = newArr.filter(item => item.id === userInfo.id)
                            if (selectedRefense && selectedRefense.length > 0) {
                                obj.isCheck = true
                            } else {
                                obj.isCheck = false
        ...
        handleAttackEdit(obj) {
            ...
            let changeAttackForm = {
                ...
                other_attrack_way: obj.attack_mode_content == null ? '' : obj.attack_mode_content,
                target_ips: obj.target_ip,
                attrack_ips: obj.attack_ip ? obj.attack_ip : '',
                step: obj.step,
                gain: obj.gain,
                remark: obj.remark
            }
            console.log(changeAttackForm)
