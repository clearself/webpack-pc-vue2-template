    ...
    <div class="list">
        <div class="ub mb-1" style="justify-content: space-around;">
            <div class="ub ub-f1 chart-box mr-1">
                <div class="ub ub-f1 ub-ac statistic-item">
                    <div class="ub ub-ver">
                        <div class="ml-2" style="font-size:12px"><i class="iconfont icon-gongjifangchengguo-gongjizongliang" style="font-size:12px;color:#cf68e6;margin-right:5px"></i>攻击总量</div>
            ...
            <div class="ub ub-f1 chart-box mr-1">
                <div class="ub ub-f1 ub-ac statistic-item">
                    <div class="ub ub-ver">
                        ...
                        <div style="color:#f29960;margin-left:35px;font-size: 20px;margin-top:5px">{{top_data.num2}}</div>
                    </div>
            ...
            <div class="ub ub-f1 chart-box mr-1">
                <div class="ub ub-f1 ub-ac statistic-item">
                    <div class="ub ub-ver">
                        ...
                        <div style="color:#61cfa9;margin-left:35px;font-size: 20px;margin-top:5px">{{top_data.num3}}</div>
                    </div>
                    <div class="ub-f1 text-right" style="padding-right: 0px;padding-top: 3px;">
        ...
        <div class="list-container" style="margin-top:-2px;box-shadow:none">
            <div class="ub ub-pj w100 mb-1 ub-ac">
                <div class="list-tips">攻击方成果(裁判)</div>
                <div>
                    <el-button icon="iconfont icon-daochu" size="small" type="primary"  @click="downData" v-per="['defense_gjcp_exp']">导出成果</el-button>
                </div>
            </div>
            <el-table class="bigTable" v-loading="loading" style="margin-top: 10px;" :data="tableData" border  stripe :height="tabHeight" :row-style="{ height: '32px' }" :header-row-style="{ height: '32px' }" :row-class-name="tableRowClassName">
                ...
                <el-table-column align="center" fixed="right" label="操作" width="160px" class-name="deepBg">
                    <template slot-scope="scope">
                        <el-button @click="handleAttackSee(scope.row)" size="mini" type="text" v-per="['defense_gjcp_view']">查看</el-button>
                        <el-button :disabled="scope.row.isCheck" @click="handlecheck(scope.row)" size="mini" type="text" v-per="['defense_gjcp_verify']">审核</el-button>
                        <el-button :disabled="scope.row.isCheck" @click="handleReject(scope.row)" size="mini" type="text" v-per="['defense_gjcp_reject']">驳回</el-button>
                    </template>
                </el-table-column>
            </el-table>
...
import {
    get_attrack_referee,
    get_attrack_match_list,
    sure_attrack_check,
    attackRefereeExportExcel
...
export default {
    ...
    methods: {
        ...
        downData() {
            attackRefereeExportExcel({ queryData: {}, paramsData: {}}).then(res => {
                console.log(res)
            }).catch(err => {
                console.log(err)
            })
        ...
        get_attack_record() {
            ...
            get_attrack_referee(data).then((res) => {
                this.loading = false
        ...
        initMatch() {
            ...
            get_attrack_match_list(data).then((res) => {
