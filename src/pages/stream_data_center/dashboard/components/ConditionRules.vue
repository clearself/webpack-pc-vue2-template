                        ...
                        <div v-else>
                            <div class="ub ub-ac" :class="{'right-first': ix===0}" style="margin-bottom: 10px">
                                ...
                                <el-input
                                    ...
                                    size="mini"
                                    clearable
                                    @blur="judge($event.target.value)"
                                    style="width: 180px"></el-input>
                                <i v-if="ix>=1" style="cursor: pointer;font-size:16px;margin-left: 5px" class="el-icon-close" @click="deleteItem(item, ix)"/>
                            </div>
                            <div v-if="ix === item.children.length-1" class="ub btn">
...
* 枚举 5： =， ！=
* */
import { getFieldByAll } from '@/server/stream_data_center/dash_board.js'
export default {
    name: 'ConditionRules',
    ...
    props: ['treeData', 'innerFlag', 'eventType'],
    created() {
    ...
    methods: {
        judge(value) {
            let ipRules = new RegExp(/^((25[0-5]|2[0-4][0-9]|[0-1]{1}[0-9]{2}|[1-9]{1}[0-9]{1}|[1-9])\.(25[0-5]|2[0-4][0-9]|[0-1]{1}[0-9]{2}|[1-9]{1}[0-9]{1}|[1-9]|0)\.(25[0-5]|2[0-4][0-9]|[0-1]{1}[0-9]{2}|[1-9]{1}[0-9]{1}|[1-9]|0)\.(25[0-5]|2[0-4][0-9]|[0-1]{1}[0-9]{2}|[1-9]{1}[0-9]{1}|[0-9]))$/).test(value)
            if (!ipRules) {
                this.$message({ type: 'warning', message: '请输入正确的IP' })
            }
        },
            ...
            getFieldByAll(data)
                ...
                .then(res => {
                    ...
                    if (res.length > 0) {
                        ...
                        this.optionsData = this.$lodash.cloneDeep(this.selectData)
                    }
                    console.log(this.selectData)
                })
                .then(() => {
