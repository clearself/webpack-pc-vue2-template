    ...
    <div>
                ...
                <el-tree
                    ...
                    <span class="custom-tree-node ub" slot-scope="{ node, data }" @mouseenter="mouseenter(data)" @mouseleave="mouseleave(data)">
                        <el-tooltip class="item" effect="dark" :content="node.label" placement="top">
                            <div class="node-label" style="color: #36373a; font-size: 12px;margin-right:10px;width:50px !important;">{{
                                node.label
                            }}</div>
                        </el-tooltip>
                        <div class="float:right">
                            <el-tooltip class="item" effect="dark" content="编辑" placement="top">
                                <span style="color: #b7c2d4; font-size: 12px;margin-right:6px" class="node-edit iconfont icon-bianji1" v-show="data.show&&data.grade!==1" @click="editNode( data )"></span>
                            </el-tooltip>
                            <el-tooltip class="item" effect="dark" content="删除" placement="top">
                                <span style="color: #b7c2d4; font-size: 12px" class="node-del iconfont icon-shanchu3" v-show="data.show&&data.grade!==1" @click="delNode(data)"></span>
                            </el-tooltip>
                        </div>
                    </span>
                </el-tree>
        ...
        <el-dialog :append-to-body="true" :title="addForm.id?'编辑分组':'添加分组'" :visible.sync="addDialog" width="700px" custom-class="common-dialog">
            ...
            <span slot="footer" class="dialog-footer">
                <el-button size="small" @click="submitCloes()">取 消</el-button>
...
<script>
import { findSearchType, saveSearchType, deleteSearchType } from '@/server/stream_data_center/chart_record.js'
// import { findSearchType, saveSearchType, deleteSearchType } from '@/server/stream_data_center/data_manage.js'
...
export default {
    ...
    methods: {
        ...
        editNode(node) {
            ...
            this.addForm.parentId = node.parentId ? node.parentId : ''
        ...
        editDisabled(arr) {
            ...
            for (let i = 0; i < arr.length; i++) {
                ...
                if (arr[i].child ? arr[i].child.length > 0 : false) {
