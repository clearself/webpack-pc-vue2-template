        ...
        >
            <el-collapse v-model="activeName" accordion @change="selectChange">
                ...
                <el-collapse-item title="最近" name="2">
                    <div class="recently">
                        ...
                        <div class="ub">
                            ...
                            <el-select size="mini" style="width:85px;box-shadow: inset 0px 0px 7px 0px #389bf7;background: rgba(0,198,255,.15);" v-model="timeUnit" placeholder="请选择">
                                <el-option
                ...
                <el-collapse-item title="自定义时间范围" name="3">
                    <div style="height: 400px" class="customize">
                        <el-date-picker
...
<script>
import { saveLogTime, getLogTime, deleteLogTime } from '@/server/stream_data_center/data_manage.js'
export default {
    name: 'CustomDate',
    ...
    mounted() {
        ...
        this.dateData = this.$getsessionStorage('saveDate').title ? this.$getsessionStorage('saveDate').title : this.$getsessionStorage('saveDate').value.join(',')
    },
    computed: {
    ...
    methods: {
        ...
        initData() {
            ...
            if (!this.emptyData && !this.chartTimeRange.length) {
            ...
            // if (this.chartTimeRange.length) {
            //     console.log('传过来时间值了吗', this.chartTimeRange)
            //     if (this.chartTimeRange[2]) {
            //         this.dateData = this.chartTimeRange[2]
            //     } else {
            //         this.dateData = this.chartTimeRange.join('~')
            //     }
            // }
        },
        handleDel(list, item) {
        ...
        handleOther(data, index) {
            // this.$parent.actualTime = data.time
            // this.$parent.timeUnit = data.timeUnit
            this.$setsessionStorage('temporary', { actualTime: data.time, timeUnit: data.timeUnit + 1 })
        ...
        handleRelative(data, index) {
            // this.$parent.actualTime = ''
            // this.$parent.timeUnit = ''
            this.$setsessionStorage('temporary', { actualTime: '', timeUnit: '' })
        ...
        addTime() {
            ...
            saveLogTime(data).then(res => {
                ...
                // this.$parent.actualTime = this.actualTime
                // this.$parent.timeUnit = this.timeUnit
                this.$setsessionStorage('temporary', { actualTime: this.actualTime, timeUnit: this.timeUnit + 1 })
                this.handleUse()
                this.$refs.datePopver.doClose()
                setTimeout(() => {
                    this.getLogTimeData()
