    ...
    <div v-loading="isView" class="flow-containers" :class="{ 'view-mode': isView }">
        <el-container style="height: 100%">
                ...
                <div style="display: flex; padding: 5px 0px; justify-content: space-between;">
                    <div>
                        <el-upload action="" :before-upload="openBpmn" style="margin-right:10px; display:inline-block;">
                            <el-tooltip effect="dark" content="加载xml" placement="bottom"><el-button class="operate-btn" size="mini" icon="el-icon-folder-opened" /></el-tooltip>
                        </el-upload>
                        ...
                        <el-tooltip effect="dark" content="预览" placement="bottom"><el-button class="operate-btn" size="mini" icon="el-icon-view" @click="preView" /></el-tooltip>
            ...
            <el-container style="align-items: stretch" class="cav-container">
                <el-main style="padding: 0;background-color: #FBFBFB;" @mousewheel.prevent><div ref="canvas" class="canvas" /></el-main>
                <el-aside class="panel-wrapper"><panel v-if="modeler" :disabled="disabled" :modeler="modeler" :users="users" :groups="groups" :categorys="categorys" /></el-aside>
            </el-container>
        </el-container>
        <el-dialog :visible.sync="xmlVisible" title="XML" :fullscreen="false" top="10vh">
            <vue-ace-editor v-model="process.xml" @init="editorInit" lang="xml" theme="chrome" width="100%" height="400" :options="{ wrap: true, readOnly: true }"></vue-ace-editor>
...
export default {
    ...
    methods: {
        ...
        addOverLay(nodeId) {
            ...
            this.nodeList = elementRegistry.filter(item => item.type === 'bpmn:UserTask' || item.type === 'bpmn:Task')
        ...
        overlayClick() {
            ...
            $('.djs-container').on('click', '.djs-overlays', e => {
                ...
                console.log(shapeId)
                const temp = this.nodeList.filter(item => item.id === shapeId)[0]
                console.log(temp)
        ...
        adjustPalette() {
            ...
            try {
                ...
                for (var gKey in allGroups) {
                    ...
                    for (var cKey in group.children) {
                        ...
                        console.log('ccccccccccccccccccccccc', control, 1111)
                        console.log($(group).attr('data-group'), 1)
                        ...
                        if (control.className && control.dataset && control.className.indexOf('entry') !== -1) {
                            ...
                            if (controlProps.icon || controlProps.title) {
                                if (groupName == 'event' || groupName == 'activity') {
                                    controlStyle.background = '#387DEE'
                                    control.innerHTML =
                                        '<div style="width:20px;height:20px;background-color: #fff;" class="' +
                                        controlProps.icon +
                                        '"></div><div class="tool-text" style="font-size: 14px;font-weight:500;margin-left:10px;color: #fff!important;">' +
                                        controlProps.title +
                                        '</div>'
                                } else {
                                    control.innerHTML =
                                        '<div style="width:20px;height:20px;" class="' +
                                        controlProps.icon +
                                        '"></div><div class="tool-text" style="font-size: 14px;font-weight:500;margin-left:10px;">' +
                                        controlProps.title +
                                        '</div>'
                                }
            ...
            if (!$('#event')[0]) {
                let title1 = $(
                    `<div id="event" style="margin-top: 20px; margin-bottom: 10px;width: 100%;height: 22px; display: flex; align-items: center; justify-content: left;"><span style="width: 4px; background: #387DEE; border-radius: 4px; height: 18px;"></span><span style="margin-left: 8px; color: rgba(0,0,0,.9); font-size: 13px;display: inline-block;height: 100%;line-height: 22px;">默认节点</span></div>`
                )
                $('div[data-group="event"]').prepend(title1)
                $('#event').click(() => {})
            }
            // 列表添加组样式
            let listData = this.$getsessionStorage('customAutoList')
            listData.forEach(group => {
                if (!$(`#customGroupId${group.id}`)[0]) {
                    let title = $(
                        `<div id="customGroupId${
                            group.id
                        }" style="margin-top: 20px; margin-bottom: 10px;width: 100%;height: 22px; display: flex; align-items: center; justify-content: left; cursor: pointer;"><span style="width: 4px; background: #DCDCDC; border-radius: 4px; height: 18px;"></span><span style="margin-left: 8px; color: rgba(0,0,0,.9); font-size: 13px;display: inline-block;height: 100%;line-height: 22px;">${
                            group.groupName
                        }</span><span style="flex: 1; text-align: right;"><i id="custom${
                            group.id
                        }Icon" style="color: #DCDCDC; display: inline-block;" class="iconfont icon-dingbuxiala"></i></span></div>`
                    )
                    $(`div[data-group="custom${group.id}"]`).prepend(title)
                    $(`#customGroupId${group.id}`).click(() => {
                        let dis = $('.djs-palette')
                            .find(`div[data-group="custom${group.id}"]`)
                            .children('.entry')[0].style.display
                        // let childLength = $('.djs-palette')
                        //     .find(`div[data-group="custom${group.id}"]`)
            ...
            // 左侧列表阻止冒泡，使其可以滚动
            $('.djs-palette')[0].addEventListener(
                'mousewheel',
                function(e) {
                    console.log(111)
...
.flow-containers {
    ...
    .djs-palette {
        .djs-palette-entries {
            ...
            .group[data-group='customplugging'] {
                display: flex;
                justify-content: center;
                align-items: center;
                flex-direction: column;
            }
            .group[data-group='customWhite'] {
                display: flex;
                justify-content: center;
                align-items: center;
                flex-direction: column;
